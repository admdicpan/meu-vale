// ==================== CONFIGURAÇÃO ====================
let API_URL = ''; // será preenchida dinamicamente via /config
...
// Carrega a configuração da API e depois segue com a inicialização da UI
...
/**
 * Busca a URL da API no endpoint /config (Cloudflare Function).
 * Se falhar (ex: desenvolvimento local), usa um fallback opcional.
 */
async function carregarConfiguracao() {
    try {
        ...
        console.log('API_URL configurada via Cloudflare');
    } catch (err) {
        console.warn('Não foi possível obter /config. Usando fallback local (apenas desenvolvimento).');
        // Para desenvolvimento local, você pode definir temporariamente uma URL abaixo:
        API_URL = 'https://script.google.com/macros/s/SUA_URL_LOCAL/exec'; 
        // ⚠️ Remova essa linha em produção ou use um fallback vazio e trate o erro.
    }
}
...
async function buscarVales() {
    if (!API_URL) {
        mostrarErro('Configuração ainda não carregada. Aguarde ou recarregue a página.');
        return;
    }
